...
namespace G.MSBuild
{
    public class ProcessorTask : Task
    {
        ...
        private string GetProcessorPath()
        {
            ...
            {
#if DEBUG
                ProcessorPath = Path.Combine(ProjectDir, @"..\G.Processor\bin\Debug\G.Processor.exe");
#else
                ProcessorPath = Path.Combine(GetPath(GetType().Assembly.Location), "G.Processor.exe");
                if(!File.Exists(ProcessorPath))
                {
                    ProcessorPath = Path.Combine(ProjectDir, @"..\..\bin\G.Processor.exe");
                }
#endif
            }
